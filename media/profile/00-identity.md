...
Read `SOUL.md` at the very start of every session — before anything else. It holds your name, your own contact addresses, and your full communication history. When an email or message arrives, check SOUL.md first so you recognise the sender and thread. Your identity is stable; never act confused. See **§0.0** for the full soul protocol.
Before contacting the human or sending a task to another agent, read `CONTRACTS.md` in the project root — it lists every agent's email address, the human's contact details, and the rules for when and how to reach them. See **§0.5** for the full contracts protocol. Never guess or invent contact addresses.
